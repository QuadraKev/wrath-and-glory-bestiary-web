...
- **Commits**: GitHub Actions workflow (`.github/workflows/deploy.yml`) handles Pages deploys on push to main. Use `--author="QuadraKev-bot <261695385+QuadraKev-bot@users.noreply.github.com>"` on all commits to differentiate AI-assisted work from the user's personal commits (standardized across both apps 2026-06-13).
- **GitHub Issues/PRs/Comments**: Use the QuadraKev-bot account (PAT in `$env:GH_TOKEN`) and include a note (e.g., "Created by Claude Code") to indicate AI authorship. (The QuadraKev-bot account's earlier TOS flag was cleared with GitHub on 2026-05-29; it is now the account to use for AI-assisted work.)
- **Auth / PAT hygiene**: The PAT lives ONLY in the Windows `$env:GH_TOKEN` env var. NEVER embed it in a remote URL, file, commit, or session log, and NEVER run commands that print it (`git remote -v`, `git config --get remote.origin.url`, `env`, etc.). Remotes use tokenless `https://github.com/...` URLs; a git credential helper (`credential.https://github.com.helper = !/root/.git-credential-ghtoken.sh`) supplies the token from `$GH_TOKEN` at runtime. When running git in WSL, propagate the token with `WSLENV=GH_TOKEN/u` (set in PowerShell before invoking `wsl`); the helper reads it from the env. Pipe any git/gh output through `sed -E 's/(ghp|github_pat)_[A-Za-z0-9_]+/[REDACTED]/g'` as a safety net. For the GitHub API, pass the token only in the `Authorization` header (never on the command line/URL).
